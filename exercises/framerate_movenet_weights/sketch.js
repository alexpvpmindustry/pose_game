...
function drawStatusIndicators(pose) {
  ...
  if (showCelebration && currentTime - celebrationTime < 900) {
    drawCelebrationEffect(currentTime);
  } else if (currentTime - celebrationTime >= 900) {
    showCelebration = false;
  }
}
...
function drawArmStatusIndicator(shoulderPoint, status, side, currentTime) {
  ...
  if (currentTime - statusChangeTime < 400) {
...
function drawCelebrationEffect(currentTime) {
  ...
  let pulseScale = 1 + 0.2 * sin((currentTime - celebrationTime) * 0.01);
...
function drawKeypoints(pose) {
  ...
  let eye_dist = 5;
  let eye_size = 20;
  // measure the distance between two eyes, set the eye_size to be the same size as the eye_dist
  if (pose['left_eye'] && pose['right_eye']) {
    eye_dist = dist(pose['left_eye'].x, pose['left_eye'].y,
      pose['right_eye'].x, pose['right_eye'].y);
    eye_size = Math.max(eye_dist - 3,3);
  }
  ...
  for (let [keypoint, color] of Object.entries(keypointColors)) {
    if (pose[keypoint] && pose[keypoint].confidence > 0.1) {
      ...
      else if (keypoint.includes('ear')) size = 8;
      else if (keypoint.includes('eye')) {
        push();
        noFill();
        stroke(0, 0, 0);    // black outline
        strokeWeight(3);
        circle(pose[keypoint].x, pose[keypoint].y, eye_size);
        pop();
        continue;
      }
